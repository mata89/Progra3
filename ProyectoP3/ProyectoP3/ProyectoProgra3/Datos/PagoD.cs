...
namespace Datos
{
    class PagoD
    {
        ...
        public List<PagoL> obtenerPagos()
        {
            ...
            try
            {
                ...
                if (this.cnx.IsError == false)
                {
                    ...
                    {
                        retorno.Add(new PagoL(fila["id_pago"].ToString(),DateTime.Parse(fila["fecha_corte"].ToString()),fila["descripcion"].ToString(),double.Parse(fila["totalPagoNeto"].ToString()),fila["creado_por"].ToString(),DateTime.Parse(fila["fecha_creacion"].ToString()), fila["modificado_por"].ToString(), DateTime.Parse(fila["fecha_modificacion"].ToString())));
                    }
                }
                else {
                    this.error = true;
        ...
        public void agregarPago(PagoL pPago)
        {
            try
            {
                string sql = "insert into pago(id_pago,fechaCorte,descripcion,totalPagoNeto,creado_por,fecha_creacion,modificado_por,fecha_modificacion) " +
                             " values(:id_pago,:fechaCorte,:descripcion,:totalPagoNeto,:creado_por,:fecha_creacion,:modificado_por,:fecha_modificacion)";
        ...
        public void editarPago(PagoL pPagoEditado, PagoL pPagoOriginal)
        {
            try
            {
                string sql = "update pago " +
                             "set (id_pago=:id_pago,fechaCorte=:fechaCorte,descripcion=:descripcion,totalPagoNeto=:totalPagoNeto,modificado_por=:modificado_por,fecha_modificacion=:fecha_modificacion" +
                ...
                parametros[4].Value = pPagoEditado.Modificado_por;

                parametros[5] = new OracleParameter();
                parametros[5].OracleType = OracleType.Timestamp;
                parametros[5].ParameterName = ":fecha_modificacion";
                parametros[5].Value = pPagoEditado.Fecha_modificacion;

                parametros[6] = new OracleParameter();
                parametros[6].OracleType = OracleType.VarChar;
                parametros[6].ParameterName = ":id_usuarioOriginal";
                parametros[6].Value = pPagoOriginal.Id_pago;
